...
function Get-NetworksWMI {
<#
    .SYNOPSIS
    Checks a remote server to see if the debugger for specific
    binaries has been set through querying the remote registry using WMI-Object.
    ...
    .EXAMPLE
    Get-DomainNetworksWMI -UserName DOMAIN\admin -Password 12456
#>
    [CmdletBinding()]
    param(
        [Parameter(ValueFromPipeline=$True)]
    ...
    process {
        ...
        $Keys = @("SOFTWARE\Microsoft\Windows NT\CurrentVersion\NetworkList\Signatures\Unmanaged")
        #SOFTWARE\Microsoft\Windows NT\CurrentVersion\NetworkCards", "SYSTEM\CurrentControlSet\services\Tcpip\Parameters\Interfaces", 
        ...
        try{
            ...
            $test | Foreach-Object {
 		        $wmi = get-wmiobject -list "StdRegProv" -namespace root\default -computername $HostName -credential $credential
 		        #($wmi.EnumKey($hklm,$key)).snames
 		        $result = ($wmi.EnumKey($hklm,$key)).snames
            }
                ...
                if($result){
                    ...
                    #$out | add-member Noteproperty 'Key:' $_
                    #$out | add-member Noteproperty 'Sub-Keys:' $result
                    #Write-Output $out
                    ...
                    $out  = New-Object -Type PSObject
                        ...
                        $result | ForEach-Object {
                            ...
                            $wmi = get-wmiobject -list "StdRegProv" -namespace root\default -computername $HostName -credential $credential
                            $subkeyresult = ($wmi.EnumValues($hklm,$subkey)).snames
                            $subkeytype = ($wmi.EnumValues($hklm,$subkey)).types
                            
                                #Now Decode the values of said subkey Values
                                if($subkeyresult){
                                    #$subkeyresult | ForEach-Object{
                                    $subkeyvaluename = $subkeyresult
                                    $out | Add-Member -PassThru NoteProperty HostName $HostName.ToUpper()
                                    for($x=0;$x -lt $subkeyvaluename.length; $x++){ 
                                    # Used Switch and Case to findand match type val
                                    # note you must eval the type of the name since switch evals arrays   
                                        switch ($subkeytype[$x]){
                                            1{
                                            $StringValue = ($wmi.GetStringValue($hklm,$subkey,$subkeyvaluename[$x])).sValue
                                            $out | add-member -PassThru Noteproperty $subkeyvaluename[$x] $StringValue
                                            #Build Dictonary for later use
                                            }

                                            2{
                                            $ExpandedStringValue = ($wmi.GetExpandedStringValue($hklm,$subkey,$subkeyvaluename[$x])).sValue
                                            $out | add-member -PassThru Noteproperty $subkeyvaluename[$x] $ExpandedStringValue
                                            #Build Dictonary for later use
                                            }

                                            3{
                                                if($subkeyvaluename[$x] -eq 'DefaultGatewayMac'){
                                                        #Check for binary hex and convert that string to a base16 int
                                                        $BinaryValue2 = ($wmi.GetBinaryValue($hklm,$subkey,$subkeyvaluename[$x])).uValue
                                                        $count = 0
                                                        $BinaryValue2 | ForEach-Object {                                                                                                             
                                                            $DefaultGatewayMac += '{0:X2}' -f $_
                                                            if ($count -lt 5){
                                                                $DefaultGatewayMac+= ":"
                                                                }
                                                            $count = $count + 1
                                                            }
                                                        $out | add-member -PassThru Noteproperty $subkeyvaluename[$x] $DefaultGatewayMac 
                                                        $DefaultGatewayMac = $null
                                                    
                                                    }
                                                else
                                                {
                                                    $BinaryValue = ($wmi.GetBinaryValue($hklm,$subkey,$subkeyvaluename[$x])).uValue
                                                    $out | add-member -PassThru Noteproperty $subkeyvaluename[$x] $BinaryValue
                                                    #Build Dictonary for later use
                                                }
                                            }

                                            4{
                                            $DWORDValue = ($wmi.GetDWORDValue($hklm,$subkey,$subkeyvaluename[$x])).uValue
                                            $out | add-member -PassThru Noteproperty $subkeyvaluename[$x] $DWORDValue
                                            #Build Dictonary for later use
                                            }

                                            7{
                                            $MultiStringValue = ($wmi.GetMultiStringValue($hklm,$subkey,$subkeyvaluename[$x])).sValue
                                            $out | add-member -PassThru Noteproperty $subkeyvaluename[$x] $MultiStringValue
                                            }
                                            default {"The subkey values name value could not be det?"}
                                         }
                                    }
                                     
                                   $outtofile = $out | ConvertTo-JSON
                                   $outtofile | Out-File -append  test.txt
                                }
...
function Get-DomainNetworksWMI {
<#
    .SYNOPSIS
...
#>
    ...
    $Computers | Invoke-Ping | ForEach-Object {
        Get-NetworksWMI -HostName $_ -UserName $UserName -Password $Password
        Write-Host $_
    }
}
